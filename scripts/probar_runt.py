"""Sonda exploratoria: consulta ciudadana de vehículo en el portal público del RUNT.

https://portalpublico.runt.gov.co/#/consulta-vehiculo/consulta/consulta-ciudadana

SPA Angular con HASH routing: NUNCA esperar `networkidle` (la app mantiene
conexiones abiertas). Se carga con `domcontentloaded` y se espera el render
por selectores. Los selectores exactos se DESCUBREN con esta sonda (dumps
paso a paso en descargas_runt/) — de aquí salen los definitivos de
Funciones/bot_runt.py.

El portal pide PLACA + CÉDULA del propietario y devuelve los datos del
vehículo + vigencia del SOAT (Mintransporte lo describe como consulta
ciudadana pública por placa). Genera PDF de la consulta.

Uso (desde integrappi/):
    python scripts/probar_runt.py EYX243 15887928 --solo-formulario  # sin captcha: valida estructura
    python scripts/probar_runt.py EYX243 15887928                    # completo (gasta 1 solve de 2Captcha)
    python scripts/probar_runt.py EYX243 15887928 --token TOKEN      # token resuelto a mano
    python scripts/probar_runt.py EYX243 15887928 --headless         # sin ventana
"""
...
PORTAL = "https://portalpublico.runt.gov.co/#/consulta-vehiculo/consulta/consulta-ciudadana"
SALIDA = Path(__file__).resolve().parents[1] / "descargas_runt"
TIMEOUT_MS = 45000
CAPTCHA_BASE = os.getenv("SEGURIDAD_RUNT_CAPTCHA_URL", "https://2captcha.com").rstrip("/")
# API_KEY_CAPTCHA es la key ya usada por los demás bots del proyecto.
CAPTCHA_KEY = os.getenv("SEGURIDAD_RUNT_CAPTCHA_KEY", "").strip() or os.getenv("API_KEY_CAPTCHA", "").strip()
CAPTCHA_TIMEOUT_S = float(os.getenv("SEGURIDAD_RUNT_CAPTCHA_TIMEOUT_S", "90"))
...
async def main(placa: str, cedula: str, solo_formulario: bool, token_manual: str | None, headed: bool) -> None:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await pagina.wait_for_timeout(4000)  # render Angular
            ...
            # Paso 2: localizar el formulario por heurística (placa + cédula).
            # Angular no garantiza ids estables: se busca por placeholder/atributos.
            entrada_placa = pagina.locator(
                "input[placeholder*='placa' i], input[id*='placa' i], input[name*='placa' i], input[formcontrolname*='placa' i]"
            ).first
            entrada_cedula = pagina.locator(
                "input[placeholder*='ula' i], input[placeholder*='identifica' i], input[id*='cedula' i], "
                "input[id*='documento' i], input[name*='cedula' i], input[formcontrolname*='cedula' i], "
                "input[formcontrolname*='documento' i]"
            ).first
            try:
                await entrada_placa.wait_for(state="visible", timeout=15000)
                await entrada_cedula.wait_for(state="visible", timeout=5000)
            except Exception:
                ...
                raise RuntimeError(
                    "No se encontró el par placa+cédula con las heurísticas. "
                    "Revise paso1_boot.html: quizá hay una pantalla previa (términos/selección de tipo)."
                )
            await entrada_placa.fill(placa.upper())
            await entrada_cedula.fill(cedula)
            ...
            print(f"[2] formulario llenado: placa={placa.upper()} cédula={cedula}")
            print(f"    placa input: {(await entrada_placa.evaluate('e => e.outerHTML.slice(0, 200)'))}")
            print(f"    cédula input: {(await entrada_cedula.evaluate('e => e.outerHTML.slice(0, 200)'))}")

            if solo_formulario:
                # El captcha es una imagen data:image/png;base64 (única del DOM):
                # guardarla para OCR/inspección — de aquí sale la estrategia de solve.
                ...
                if img:
                    ...
                    print(f"[captcha-img] imagen guardada: descargas_runt/captcha_{placa}.png ({len(b64)//1024} KB b64)")
                print("[fin] --solo-formulario: sin submit. Dumps en descargas_runt/")
                return

            # Paso 3: captcha de IMAGEN propia del portal (data:image/png;base64,
            # única del DOM). Se resuelve por 2Captcha (method=base64, ~US$0.001)
            # o con --token leído a mano desde la imagen guardada.
            ...
            if not captcha_texto:
                ...
                if not img:
                    (SALIDA / "paso3_sin_captcha_img.html").write_text(await pagina.content(), encoding="utf-8")
                    raise RuntimeError("No se encontró la imagen del captcha (data:image) en el DOM")
                b64 = img.split(",", 1)[-1]
                (SALIDA / f"captcha_{placa}.png").write_bytes(__import__("base64").b64decode(b64))
                captcha_texto = await asyncio.to_thread(resolver_captcha_imagen, img)
            await pagina.fill("input[formcontrolname=captcha]", captcha_texto)
            print(f"[3] captcha de imagen resuelto ({'manual' if token_manual else '2Captcha'})")

            # Paso 4: submit + trampa de descarga (el portal genera PDF de la consulta).
            pdf_bytes = b""
            ...
            # Angular renderiza el resultado client-side: esperar contenido nuevo.
            ...
            if pdf_bytes:
                (SALIDA / f"paso5_resultado_{placa}.pdf").write_bytes(pdf_bytes)

            # Paso 5b: la página de resultado es un ACORDEÓN de mat-expansion-panel
            # con carga perezosa: expandir SOAT (y RTM) para ver la póliza real.
            for titulo_panel in ("Póliza SOAT", "RTM"):
                try:
                    panel = pagina.locator("mat-expansion-panel-header", has_text=titulo_panel).first
                    await panel.click(timeout=8000)
                    await pagina.wait_for_timeout(2500)  # carga Ajax del panel
                except Exception as exc:
                    print(f"[5b] panel '{titulo_panel}' no se pudo expandir: {exc}")
            await pagina.wait_for_timeout(1500)
            (SALIDA / "paso6_paneles_abiertos.html").write_text(await pagina.content(), encoding="utf-8")
            await pagina.screenshot(path=str(SALIDA / "paso6_paneles_abiertos.png"), full_page=True)
            ...
            for leyenda in ("SOAT", "no se encuentra", "no registra", "sin información", "propietario"):
            ...
            print("\nDumps en descargas_runt/ (paso5_resultado.html es la página de resultado).")
        finally:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Sonda del portal público del RUNT (consulta ciudadana de vehículo)")
    ...
    if not args.solo_formulario and not args.token and not CAPTCHA_KEY:
        print("ERROR: falta SEGURIDAD_RUNT_CAPTCHA_KEY (o API_KEY_CAPTCHA) en .env — o use --token/--solo-formulario")
        sys.exit(2)
    asyncio.run(main(args.placa, args.cedula, args.solo_formulario, args.token, headed=not args.headless))
