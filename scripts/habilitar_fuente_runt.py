"""Habilita la fuente "runt" en el config de las empresas existentes (2026-08-30).

Las empresas creadas ANTES de la fuente runt tienen `config.fuentes_habilitadas`
persistido sin ella (el default viejo era [manifiestos_rndc, procuraduria]).
Este script la agrega con $addToSet (idempotente, sin carrera). El gate real
del consumo sigue siendo el PLAN: agregar la fuente al config es inofensivo
...
Uso:
    python scripts/habilitar_fuente_runt.py             # dry-run
    python scripts/habilitar_fuente_runt.py --ejecutar  # real
"""
...
def main(ejecutar: bool) -> None:
    ...
    for empresa in col_empresas.find({"activo": True}):
        ...
        if fuentes is None:
            ...
            continue
        if "runt" in fuentes:
            print(f"{empresa.get('nombre')}: ya tiene runt ✓")
            continue
    ...
    for empresa in pendientes:
        ...
        print(f"{empresa.get('nombre')}: {actual} → {actual + ['runt']}")
        if ejecutar:
            col_empresas.update_one(
                {"_id": empresa["_id"]},
                {"$addToSet": {"config.fuentes_habilitadas": "runt"}},
            )
            print("  ✔ habilitada")
    ...
    if not ejecutar:
        ...
        print(f"\nListo: {len(pendientes)} empresa(s) habilitadas con la fuente runt.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Habilitar la fuente runt en empresas existentes")
