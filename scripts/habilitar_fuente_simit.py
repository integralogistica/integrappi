"""Habilita la fuente "simit" en el config de las empresas existentes (2026-09-01).

Las empresas creadas ANTES de la fuente simit tienen `config.fuentes_habilitadas`
...
Uso:
    python scripts/habilitar_fuente_simit.py             # dry-run
    python scripts/habilitar_fuente_simit.py --ejecutar  # real
"""
...
def main(ejecutar: bool) -> None:
    ...
    for empresa in col_empresas.find({"activo": True}):
        ...
        if fuentes is None:
            ...
            continue
        if "simit" in fuentes:
            print(f"{empresa.get('nombre')}: ya tiene simit ✓")
            continue
    ...
    for empresa in pendientes:
        ...
        print(f"{empresa.get('nombre')}: {actual} → {actual + ['simit']}")
        if ejecutar:
            col_empresas.update_one(
                {"_id": empresa["_id"]},
                {"$addToSet": {"config.fuentes_habilitadas": "simit"}},
            )
            print("  ✔ habilitada")
    ...
    if not ejecutar:
        ...
        print(f"\nListo: {len(pendientes)} empresa(s) habilitadas con la fuente simit.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Habilitar la fuente simit en empresas existentes")
