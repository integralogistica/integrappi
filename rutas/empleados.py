...
# ——— Rutas y aplicación ———
ruta_empleado = APIRouter(prefix='/empleados', tags=['Empleados'], responses={status.HTTP_404_NOT_FOUND: {'message': 'No encontrado'}})
...
async def get_empleado_por_identificacion(identificacion: str = Query(..., description='Número de identificación')):
    condiciones = ([{'identificacion': identificacion}, {'IDENTIFICACIÓN': identificacion}]
                   if not identificacion.isdigit()
                   else [{'identificacion': identificacion}, {'identificacion': int(identificacion)}, {'IDENTIFICACIÓN': identificacion}, {'IDENTIFICACIÓN': int(identificacion)}])
...
async def enviar_certificado(identificacion: str = Query(..., description='Identificación del empleado'), req: EnviarRequest = Body(...)):
    condiciones = ([{'identificacion': identificacion}, {'IDENTIFICACIÓN': identificacion}]
                   if not identificacion.isdigit()
                   else [{'identificacion': identificacion}, {'identificacion': int(identificacion)}, {'IDENTIFICACIÓN': identificacion}, {'IDENTIFICACIÓN': int(identificacion)}])
    doc = coleccion_empleados.find_one({'$or': condiciones})
    ...
    coleccion_historial.insert_one({'identificacion': emp.identificacion, 'nombre': emp.nombre, 'fecha_solicitud': datetime.now()})
    ...
    try:
        ...
        c.drawImage(ImageReader(BytesIO(base64.b64decode(img_data))), 0, 0, width=width, height=height)
    except:
        pass
    # Estilos y contenido
    ...
    texto = f"El señor/a <b>{emp.nombre}</b>, identificado/a con cédula número <b>{ced}</b>, labora en nuestra empresa desde <b>{fecha_leg}</b>, desempeñando el cargo de <b>{emp.cargo}</b> con contrato a término <b>{emp.tipoContrato}</b>."
    ...
    auxs = [('Auxilio Vivienda', emp.auxilioVivienda), ('Auxilio Alimentación', emp.auxilioAlimentacion), ('Auxilio Movilidad', emp.auxilioMovilidad), ('Auxilio Rodamiento', emp.auxilioRodamiento), ('Auxilio Productividad', emp.auxilioProductividad), ('Auxilio Comunic', emp.auxilioComunic)]
    ...
    story.append(Spacer(1,6))
    story.append(Paragraph(f"La presente certificación se expide a solicitud del interesado, dado a {cert_date} en Bogotá.", info_style))
    story.append(Spacer(1,6))
    ...
    try:
        ...
        c.drawImage(ImageReader(BytesIO(base64.b64decode(sig))), width/2-75, 300-10, width=150, height=50, mask='auto')
    except:
        pass
    c.setFont('Times-Bold',12); c.drawCentredString(width/2,305,'PATRICIA LEAL AROCA')
    c.setFont('Times-Roman',10); c.drawCentredString(width/2,290,'Certificado laboral')
    c.drawCentredString(width/2,278,'Gerente de gestión humana'); c.drawCentredString(width/2,266,'Integra cadena de servicios')
    c.showPage(); c.save()
    # Preparar bytes
    buffer.seek(0)
    ...
    attachment = base64.b64encode(final_bytes).decode()
    payload = {'from':'no-reply@integralogistica.com','to':[emp.correo],'subject':f'Certificado Laboral - {emp.nombre}','html':f'<p>Hola {emp.nombre},</p><p>Adjunto tu certificado laboral generado automáticamente.</p>','attachments':[{'filename':f'certificado_{emp.identificacion}.pdf','type':'application/pdf','content':attachment}]}
    try:
